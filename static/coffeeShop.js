...
function displayCart(cartContents) {
  ...
  for (const [item, price] of Object.entries(cartContents)) {
    tableContents += `<tr><td><button class="remove-from-order" id="${item}"> - </button>${item}</td><td>${price}</td></tr>`;
  }

  document.querySelector('#cart-items').innerHTML = tableContents;

  const allRemoveButtons = document.querySelectorAll('.remove-from-order');
  for (const button of allRemoveButtons) {
    button.addEventListener('click', () => {
      const item = button.id;
  
      fetch(`/remove-cart.json?item=${item}`)
        .then((response) => response.json())
        .then((result) => {
          cart = result.cart;
          orderTotal = result.total;
